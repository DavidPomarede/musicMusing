#p1 >> pads([12,-5,10,8])
#d1 >> pulse([10,-6,12,1])
p2 >> pads([6,5,-2,8,6,1,2,6], dur=[1,1/2,1,1/2,1,2], amp=[1,3/4,3/4,3/4])
#d2 >> pulse([-32,6,-12,-1])
...
b1 >> bass([0,2,0,6], dur=[1,1/2,1,1/2], amp=[1,3/4,3/4,3/4])
...
p4 >> pluck([0, 0, 0,0], dur=[1, 1/2, 1, 1/2], amp=0.75) 
...
d1 >> play(P["!#$%&*0-0/1234:=@ABCDEFGHIJKLMNOPQRSTUVWXYZ\\^abcdefghijklmnopqrstuvwxyz~"].layer("mirror"),
	pan=(-1,1),
	dur=PDur(5,8),
	sample=-1,
	rate=var([1,4],[28,4]))
...
s1 >> swell((3,3,6,const(6)), dur=4) + var([])
